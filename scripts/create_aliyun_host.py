...
class AliyunRunInstancesExample(object):
    ...
    def __init__(self):
        self.access_id = 'oGJAYq6WfRLWBw08'
        self.access_secret = '1FnnaH5kPDM6Kv8xantGDeJ3r3LlwM'

        # 是否只预检此次请求。true：发送检查请求，不会创建实例，也不会产生费用；false：发送正常请求，通过检查后直接创建实例，并直接产生费用
        self.dry_run = False
        # 实例所属的地域ID
        self.region_id = 'cn-wulanchabu'
        # 实例的资源规格
        self.instance_type = 'ecs.g6.large'
        # 实例的计费方式
        self.instance_charge_type = 'PostPaid'
        # 镜像ID
        self.image_id = 'centos_7_06_64_20G_alibase_20190711.vhd'
        # 指定新创建实例所属于的安全组ID
        self.security_group_id = 'sg-0jlccsezqwg46urvwj6l'
        # 购买资源的时长
        self.period = 1
        # 购买资源的时长单位
        self.period_unit = 'Hourly'
        # 实例所属的可用区编号
        self.zone_id = 'cn-wulanchabu-a'
        # 网络计费类型
        self.internet_charge_type = 'PayByTraffic'
        # 虚拟交换机ID
        self.vswitch_id = 'vsw-0jlokqnm42me3xh4exms1'
        # 实例名称
        self.instance_name = 'launch-advisor-20201105'
        # 实例的密码
        self.password = 'L@bs&d1250'
        # 指定创建ECS实例的数量
        self.amount = 1
        # 公网出带宽最大值
